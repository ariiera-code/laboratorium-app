<div class="pt-4">
  ...
  <div class="relative">
    <div class="flex justify-between items-center mb-3">
      <div class="flex-grow h-10 flex items-center md:flex-grow-0">
        @if ($this->searchableColumns()->count())
          <div class="w-96 flex rounded-lg shadow-sm">
            <div class="relative flex-grow focus-within:z-10">
              <div class="absolute bg-gray-100 opacity-75 inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              ...
              <input wire:model.debounce.500ms="search"
                class="bg-gray-100 opacity-75 w-full pl-10 py-3 text-sm leading-4 block rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none"
        ...
        @if ($exportable)
          <div x-data="{ init() {
            ...
            <button wire:click="export"
              class="flex items-center space-x-2 px-3 border border-green-400 rounded-md bg-white text-green-500 text-xs leading-4 font-medium uppercase tracking-wider hover:bg-green-200 focus:outline-none"><span
                class="font-bold transition">{{ __('Export') }}</span>
    ...
    <div class="bg-white max-w-screen">
      <div class="rounded-lg @unless($this->hidePagination) rounded-b-none @endif">
        <div class="table align-middle overflow-x-scroll min-w-full">
          @unless($this->hideHeader)
            <div class="table-row bg-white divide-gray-200">
              @foreach ($this->columns as $index => $column)
                @if ($hideable === 'inline')
                  ...
                  <div class="table-cell overflow-hidden align-top">
                    @isset($column['filterable'])
                      @if (is_iterable($column['filterable']))
                        <div wire:key="{{ $index }}">
                          @include('datatables::filters.select', ['index' => $index, 'name' => $column['label'], 'options'
                          => $column['filterable']])
                        </div>
                      @else
                        <div wire:key="{{ $index }}">
            ...
            <div class="flex md:block items-center justify-between">
              ...
              @if (count($this->results))
                ...
                <div class="my-4 sm:my-0">
                  <div class="hidden lg:inline md:flex-1">
                ...
                <div class="flex justify-end text-gray-600 md:justify-center">
                  <p>
                    {{ __('Show') }} <span class="font-bold">{{ $this->results->firstItem() }} -
                      {{ $this->results->lastItem() }} </span>
                    {{ __('of') }}
                    <span class="font-bold">{{ $this->results->total() }}</span>
                  </p>
                </div>
              @endif
            </div>
