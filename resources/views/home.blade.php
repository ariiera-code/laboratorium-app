...
<body class="font-sans antialiased bg-gray-800">
  ...
  <nav x-data="{ open: false }" class="z-50 sticky top-0 bg-gray-800 transition">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden transition">
      ...
      @endcan
      <div class="pb-2 space-y-1">
        <x-jet-responsive-nav-link href="{{ route('biodata') }}" :active="request()->routeIs('places.*')">
          {{ __('Biodata') }}
        </x-jet-responsive-nav-link>
      </div>
      ...
      <div class="w-8/12 sm:w-full grid grid-cols-1 place-content-end sm:pt-12">
        ...
        <div class="pb-16 border-b border-gray-600 mt-16">
          ...
          <a href="{{ route('places.index') }}"
            ...
            <p class="mt-4">
              <span
                class="hidden sm:inline-block font-sans bg-indigo-400 p-2 rounded-md font-extrabold text-3xl text-white">Discover
                Now!</span>
            </p>
          </a>
        </div>
